...
public class Dog implements Pet {
    ...
    public Sayable toSayable(final int i) {
        class DogOut extends SuperDog implements Sayable {
            @Override
            ...
            @Override
            public String say() {
                String date = super.formatter.format(new Date());
                if (i < 1)
                    return getSuperQuotes() + name + getSuperQuotes() + " спит.";
                else {
                    StringBuilder sb = new StringBuilder("гав! ");
                    String say = " лает ";
                    String quote = getSuperQuotes();
                    for (int j = 1; j < i; j++) {
                        sb.insert(1, 'а');
                    }
                    return quote + name + quote + say + sb + date;
                }
            }
        }
        DogOut dogs = new DogOut();
        return dogs;
    }
}